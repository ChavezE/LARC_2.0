 ...
 */
 void leaveTerrineRight () {
    ...
    AbstractLoggable *loggerArray[2]{&lcdLogger, &serialLogger};
    Logger logger("Mega", "LeaveTerrRight", LevelLogger::INFO, loggerArray, 1);

    logger.log("Leave Terr Right");
    delay(2000);
    ...
    turnToObjectiveN(iWest);
    // Pass the gate
    backwardNCm(50, false); // TODO: Maybe is better to pass until pass the sharpLeftBack

    // Going until the terrine zone to leave it
    logger.log("Back with Left");
    delay(1000);
    int distFront, distBack;
    do {
    ...
    logger.log("Forw until Cms");
    delay(1000);
    backwardUntilWallN(distFromWall);
